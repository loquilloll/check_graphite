...
def main():
    ...
    # Debug
    if "v" in cars:
        print("time frame: " + cars["t"])
    if "v" in cars:
        print("output    : " + output)
    if "v" in cars:
        print("perfdata  : " + perfdata)
    if "v" in cars:
        print("result    : " + result)
    if "v" in cars:
        print("\n\n")
...
def getGraph(name, url, time, _user, _passwd):
    ...
    try:
        ...
        try:
            ...
            r = requests.get(url, params=payload, auth=HTTPBasicAuth(user, passwd))
...
def usage():
    print(
        "Usage: \n"
        + sys.argv[0]
        + " -g [Graph] -H [url] [-w [u][Wthreshold]] [-c [u][Cthreshold]] [-t [time frame]] [-u [username]] [-p [password]] [-h, --help]"
    )
...
def showVerboseHelp():
  ...
  """
        + sys.argv[0]
        + """ -g carbon.agents.cpuUsage -H http://example.com/ -w 85.4 -c u0 -t 3d
